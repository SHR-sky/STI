...
void TIM3_Config()
{
	...
	//TIM3时间基准配置
	TIM_TimeBaseStructure.TIM_Period = 83 ; // 100k
	TIM_TimeBaseStructure.TIM_Prescaler = 0x9;
...
void ADC2_DMA_Trig( u16 Size )                    
{
	...
	DMA2_Stream2->NDTR = Size;                             //设置数量
	DMA_ClearITPendingBit( DMA2_Stream2 ,DMA_IT_TCIF2|DMA_IT_DMEIF2|DMA_IT_TEIF2|DMA_IT_HTIF2|DMA_IT_TCIF2 );
	ADC2->SR = 0;
	DMA2_Stream2->CR |= (uint32_t)DMA_SxCR_EN;
}	
...
void ADC_Config(void)
{
	...
	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)ADC2_ConvertedValue;
	DMA_InitStructure.DMA_BufferSize = ADC2_DMA_Size;
