...
void Set_PointFre(u32 Key_Value, u8* Task_ID)
{
	...
	//固定显示
	LCD_Show_CEStr(0,0,"4351");
}
//任务

void Task0_PointFre(u32 Key_Value)//(10M) 0-100 000 000
{
	static u32 SinFre = 100000;
	u8 showstr[StrMax]={0};
...
void Task3_SweepFre(u32 Key_Value)//扫频
{
	...
	if(Key_Value != K_NO)
	{
		...
		if(Param_Mode == 3)//步进时间设置
		{
			...
			if(SweepTime>9999) SweepTime=9999;//数据限制
